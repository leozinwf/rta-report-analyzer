...
function truncateToken(token: string, max = 34): string {
  if (!token) return "N/D";
...
export function TokenCell({
  ...
  return (
    <span className={`inline-flex max-w-full items-center gap-1.5 ${full ? "" : "whitespace-nowrap"}`} onClick={stopRow}>
      <span
        className={`font-mono text-xs ${full ? "break-all" : "max-w-[280px] truncate"}`}
        ...
        <>
          ...
          {url ? (
            <a
              ...
              rel="noreferrer"
              onClick={stopRow}
              title={/^A-/i.test(value) ? "Abrir no Automation" : "Abrir no RTA"}
              aria-label="Abrir token no sistema de origem"
              className="inline-flex rounded-md border border-line p-1 text-muted hover:bg-panel-2 hover:text-accent"
            >
      ...
      <Modal open={open} title="Token" onClose={() => setOpen(false)} zIndexClass="z-[80]">
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {url ? (
              <a href={url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-line px-3 py-2 text-sm font-semibold hover:bg-panel-2">
                <ExternalLink className="size-4" /> Abrir token
              </a>
            ) : null}
          </div>
        </div>
