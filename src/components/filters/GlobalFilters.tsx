...
function ChipSelect({ label, values, selected, onChange }: { label:string; values:Array<{value:string;label:string}>; selected:string[]; onChange:(next:string[])=>void }) {
  return <label className="block min-w-40 text-xs text-muted">{label}<select value="" onChange={(event)=>{const value=event.target.value;if(value&&!selected.includes(value))onChange([...selected,value]);}} className="mt-1 w-full rounded-lg border border-line bg-panel px-2 py-1.5 text-sm text-ink"><option value="">Todos</option>{values.filter(item=>!selected.includes(item.value)).map(item=><option key={item.value} value={item.value}>{item.label}</option>)}</select>{selected.length?<div className="mt-1 flex flex-wrap gap-1">{selected.map(value=><button key={value} type="button" onClick={()=>onChange(selected.filter(item=>item!==value))} className="rounded-md bg-cyan-50 px-2 py-0.5 text-[11px] text-cyan-800">{values.find(item=>item.value===value)?.label??value} ×</button>)}</div>:null}</label>;
}
...
export function GlobalFilters() {
  ...
  return <section className="border-b border-line bg-panel/70 px-6 py-4"><div className="mb-3 flex items-center justify-between"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted">Filtros globais</p><button type="button" onClick={()=>setFilters(emptyFilters())} className="rounded-lg border border-line px-3 py-1 text-xs hover:bg-panel-2">Limpar filtros</button></div><div className="flex flex-wrap gap-3">
    ...
    <ChipSelect label="Sistema" values={platforms.map(value=>({value,label:value}))} selected={filters.platforms} onChange={values=>setFilters({...filters,platforms:values as ExecutionPlatform[]})}/>
    ...
    <ChipSelect label="Tentativa" values={attempts.map(value=>({value:String(value),label:String(value)}))} selected={filters.attempts.map(String)} onChange={values=>setFilters({...filters,attempts:values.map(Number)})}/>
