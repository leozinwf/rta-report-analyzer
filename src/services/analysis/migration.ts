...
export function analyzePlatformMigration(rows: ClassifiedExecution[], toleranceMinutes = DEFAULT_TOLERANCE_MINUTES): MigrationAnalysis[] {
  ...
  return [...byRobot.entries()].map(([robot, robotRows]) => {
    const rta = robotRows.filter(r => r.platform === "RTA");
    const automation = robotRows.filter(r => r.platform === "Automation");
    if (!rta.length) return { robot, state: automation.length ? "automation_only" : "unknown", rtaCount: 0, automationCount: automation.length, rtaAfterAutomation: 0, rtaAfterTolerance: 0, toleranceMinutes };
  ...
  }).sort((a,b) => (b.state === "inconsistent" ? 1 : 0) - (a.state === "inconsistent" ? 1 : 0) || (b.rtaAfterTolerance-a.rtaAfterTolerance));
}
