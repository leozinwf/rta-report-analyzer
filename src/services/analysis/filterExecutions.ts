...
export function filterExecutions(executions: ClassifiedExecution[], filters: GlobalFilters): ClassifiedExecution[] {
  ...
  return executions.filter((row) => {
    ...
    if (search) {
      const haystack = normalizeKey([row.id, row.robot, row.robotId, row.status, row.message, row.platform, row.tenant, row.environment, row.stage].filter(Boolean).join(" "));
