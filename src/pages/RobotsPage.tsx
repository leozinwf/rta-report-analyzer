...
export function RobotsPage() {
  ...
  return <div><SectionHeader title="Robôs" description="Types individuais e seus fornecedores/templates compartilhados." action={<div className="flex flex-wrap gap-2"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar robô ou fornecedor" className="rounded-lg border border-line bg-panel px-3 py-2 text-sm"/><select value={supplier} onChange={e=>setSupplier(e.target.value)} className="rounded-lg border border-line bg-panel px-3 py-2 text-sm"><option value="all">Todos os fornecedores</option>{suppliers.map(s=><option key={s} value={s}>{s}</option>)}</select><select value={status} onChange={e=>setStatus(e.target.value)} className="rounded-lg border border-line bg-panel px-3 py-2 text-sm"><option value="all">Todos os status</option><option value="healthy">Saudável</option><option value="watch">Atenção</option><option value="critical">Crítico</option></select></div>}/><DataTable rows={rows} columns={columns} rowKey={r=>r.id} pageSize={25} pageSizeOptions={[25,50,100,200]} onRowClick={r=>navigate(`/robos/${encodeURIComponent(r.id)}`)}/></div>;
}
